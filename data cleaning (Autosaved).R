#code to prepare the data for analysis#last edited Dec 12, 2023 by A. R. Martinig#Delete previous information stored rm(list=ls(all=T))##set wd to the folder with all your csv's in itsetwd("~/Documents/Files/Post-docs/UNSW 2022-2024/Aim 1")options(scipen=999, dplyr.width = Inf, tibble.print_min = 50, repos='http://cran.rstudio.com/') #scipen forces outputs to not be in scientific notation #dplyr.width will show all columns for head() function and tibble.print_min sets how many rows are printed and repos sets the cran mirror#load librariespackages=c("tidyverse", "dplyr", "googlesheets4") #do not put in the showtext package as it messes EVERYTHING up# Install packages not yet installedinstalled_packages <- packages %in% rownames(installed.packages())if (any(installed_packages == FALSE)) {  install.packages(packages[!installed_packages])}# Packages loadinglapply(packages, library, character.only = TRUE)# Read the Google Sheet into a dataframedf <- read.csv("Aim 1 - Aim 1.csv", header=T) %>%	filter(!composite_variable=="Y", !obsID=="TBD") %>%	mutate_if(is.character, as.factor) %>%	select(-c(title, DOI, journal, year, region, country, composite_variable, effect_size_p_value, consider_excluding, needs_second_opinion, cross_checked, data_source, comments)) %>%	#for now I am going to filter out some of the more complicated comparisons	filter(!group_1 %in% c("nonbreeder", "low", "dead")) %>%	group_by(function_needed, study_design) %>%	slice_head(n=10) %>%	droplevels() %>%
	mutate(fitness_metric_clean =
		case_when(
		#age at first reproduction - How old an individual is when they first have offspring
		#lifetime breeding success - Number of offspring produced over a lifetime
		#lifetime reproductive success - Number of weaned offspring produced over a lifetime
		#litter/clutch size - number of offspring produced at one time
		#number of reproductive attempts - Number of breeding attempts (litters, clutches, etc.) an individual has over their lifetime
		#Recruitment - Whether an individual survives until a specific life stage, often marked by surviving until they are “added” to a population
		#reproductive lifespan - Time between first and last reproductive attempt
		
		fitness_metric %in% c("survival winter", "survival to yearling", "survival to maturity", "survival spring", "survival autumn", "survival annual", "survival 1st to 2nd year", "lifespan", "age at death", "longevity") ~ "survival", 	#survival - time between birth and death
		
		
    		fitness_metric %in% c("offspring survival to yearling", "offspring survival to independence", "offspring survival to age 4", "number of fledglings that survived to breed") ~ "offspring survival", 		#Number of offspring that survive/survive to weaning/recruit/reproduce - number of offspring produced over a lifetime that survive until a set time point


    		fitness_metric %in% c("lifetime n of pups reared to weaning", "n pups reared to weaning", "number of fledglings", "annual number of fledglings", "number of offspring recruited", "number of daughters reaching sexual maturity", "n of yearling daughters", "number of fledglings per breeding attempt", "weaning success", "reproductive success (at least one fledgling)", "number of recruits per nest", "number of recruits", "breeding success (ratio of chicks fledged to number of eggs laid)")  ~ "lifetime reproductive success", #number of weaned offspring produced over lifetime - include recruitment here
