  ...
  mutate(publication_year = as.numeric(str_sub(reference, start = -4)),
	...
	fitness_metric_clean = as.factor(
		...
		#how the offspring themselves survive - often until recruitment (when they are "added" to a population) or after reaching independence from parents (i.e., post-fledging)
  		...
  		ageclass=as.factor(ifelse(fitness_metric_clean %in% c("offspring survival", "offspring reproduction"), "offspring", "adult")))
...
taxa<-Species_info%>%group_by(reference, paperID) %>%filter(row_number()==1) %>% select(reference, paperID, animal)

df_species<-left_join(df, taxa, by=c("reference", "paperID"))		
head(df_species)	

df_species%>%filter(species=="Vulpes macrotis mutica")	#export google sheet to CSV to make it easier to work with (it is a pain because it has different file formats for numeric columns, like "lists")write.csv(df_species, "~/Documents/Files/Post-docs/UNSW 2022-2024/Aim 1/Fitness-and-dispersal-MA/data/clean_data.csv")	