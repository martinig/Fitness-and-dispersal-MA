...
There are a couple of solutions to estimate species-level effect sizes. I chose the simplest one, which assumes 

(i) effect sizes are homogeneous within the same species,

(ii) effect size estimates are arbitrarily correlated within the sample species (given that I do not know to which degree they are correlated, you might want to assume a correlation coefficient),

(ii) effect sizes are independent among species.

This approach can be implemented via phylo_model(), which can be fund at the following code chunk.

```{r}
# function to estimate species-level effect sizes
phylo_model <- function(data, tree, correlation = 0.5) {
  
  # accounting for within-species correlation
  Vm <- vcalc(vi = dat$vi, cluster = dat$species_cleaned, obs = dat$effectID, rho = correlation)
  dat <- escalc(yi = yi, vi = vi, data = dat)
  dat2 <- aggregate(dat, cluster = species_cleaned, V = Vm)
  ...
  # calculate CIs
  dat3 <- dat3 %>%
    mutate(lb = yi - sqrt(vi) * qnorm(1 - 0.05/2), 
           ub = yi + sqrt(vi) * qnorm(1 - 0.05/2)) %>%
    arrange(phylogeny)
  names(dat3)[2] <- "estimate"
  # get species class data
  species_class <- dplyr::distinct(dat, phylogeny, .keep_all = TRUE) %>%
  ...
  return(list(data = dat3, class = species_class2))  # return processed data and species class
}
...
# plot tree
# I only made a basic plot  with all necessary information on it
...
p3 <- p2 + geom_facet(panel = "Effect size", data = result$data, geom = ggstance::geom_pointrangeh,
