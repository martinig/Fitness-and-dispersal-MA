---
title: "**Animal dispersal costs are not universal: a global meta-analysis**"
...
# Set seed for reproducibility (any stochastic optimizer / bootstrap step)
set.seed(2026)
```

### Create output folder for saved figures

```{r create figures folder}
# All supplementary figures and tables are saved to figures/github figures/
# with filenames that match their caption labels (e.g. Figure S23 -> s23.jpeg).
fig_dir <- here("figures", "github figures")
...
#the phylogenetic tree (always make sure you have run your species_tree script if you made any changes to the data)
...
all_models <- function(model, mod = " ", type = "normal") {
  ...
  run_rma1 <- function(name) {
    ...
    # variance covariance matrix for sampling errors
    VCV1 <- vcalc(vi = dat2$vi, cluster = dat2$shared_group, obs = dat2$effectID, rho = 0.5)
    ...
    run_rma2 <- function(name) {
            ...
            # variance covariance matrix for sampling errors
            VCVa <- vcalc(vi = dat2$vi_abs, cluster = dat2$shared_group, obs = dat2$effectID, rho = 0.5)
            #VCVa <- nearPD(VCVa)$mat
               
               rma.mv(yi_abs, V = VCVa,
               mods = ~relevel(dat2[[mod]], ref = name),
                      random = list(
                            ~ 1 | effectID,
                            ~ 1 | paperID,
...
# folded variance
# Compute the expected value of |Zr| (the absolute effect size). When you fold a normal distribution around zero (take the absolute value), the new mean shifts away from zero. This function returns that shifted mean, used for the "absolute value analysis" where we analyze the magnitude of dispersal effects regardless of direction.
...
reduced_intercept
ggsave(file.path(fig_dir, "s1.jpeg"), reduced_intercept, width = 6, height = 6, dpi = 300)
```
...
ggsave(file.path(fig_dir, "s2.jpeg"), width = 6, height = 6, dpi = 300)
...
ggsave(file.path(fig_dir, "s3.jpeg"), width = 6, height = 6, dpi = 300)
...
ggsave(file.path(fig_dir, "s4.jpeg"), width = 6, height = 6, dpi = 300)
```
...
ggsave(file.path(fig_dir, "s5.jpeg"), width = 6, height = 6, dpi = 300)
```
...
ggsave(file.path(fig_dir, "s6.jpeg"), width = 6, height = 6, dpi = 300)
```
...
ggsave(file.path(fig_dir, "s7.jpeg"), width = 6, height = 6, dpi = 300)
...
ggsave(file.path(fig_dir, "s8.jpeg"), width = 6, height = 6, dpi = 300)
```
...
ggsave(file.path(fig_dir, "s9.jpeg"), width = 6, height = 6, dpi = 300)
...
ggsave(file.path(fig_dir, "s10.jpeg"), width = 6, height = 6, dpi = 300)
```
...
ggsave(file.path(fig_dir, "s11.jpeg"), width = 8, height = 8, dpi = 300)
...
ggsave(file.path(fig_dir, "s12.jpeg"), width = 6, height = 6, dpi = 300)
...
bubble_plot(mod_dur,
                ...
                ylab = "Effect Size: Zr",
                       g = TRUE)
ggsave(file.path(fig_dir, "s13.jpeg"), width = 6, height = 6, dpi = 300)
```
...
bubble_plot(mod_dur_abs,
                ...
                ylab = "Effect Size: Zr",
                       g = TRUE)
ggsave(file.path(fig_dir, "s14.jpeg"), width = 6, height = 6, dpi = 300)
```
...
ggsave(file.path(fig_dir, "s15.jpeg"), width = 6, height = 6, dpi = 300)
...
ggsave(file.path(fig_dir, "s16.jpeg"), width = 6, height = 6, dpi = 300)
```
...
ggsave(file.path(fig_dir, "s17.jpeg"), width = 6, height = 6, dpi = 300)
```
...
ggsave(file.path(fig_dir, "s18.jpeg"), width = 6, height = 6, dpi = 300)
```
...
ggsave(file.path(fig_dir, "s19.jpeg"), width = 6, height = 6, dpi = 300)
```
...
ggsave(file.path(fig_dir, "s20.jpeg"), width = 6, height = 6, dpi = 300)
```
...
ggsave(file.path(fig_dir, "s21.jpeg"), width = 8, height = 8, dpi = 300)
```
...
# Save Table S14 as an image (kableExtra HTML table -> PNG via webshot)
kableExtra::save_kable(tab_sex_species_class, file = file.path(fig_dir, "table s14.png"), zoom = 2)
```
...
ggsave(file.path(fig_dir, "s22.jpeg"), width = 8, height = 8, dpi = 300)
...
ggsave(file.path(fig_dir, "s23.jpeg"), width = 8, height = 8, dpi = 300)
...
ggsave(file.path(fig_dir, "s24.jpeg"), width = 8, height = 8, dpi = 300)
...
figS25 <- facet_widths(p3, c(Tree = 0.6, `Effect size` = 0.4))
figS25
ggsave(file.path(fig_dir, "s25.jpeg"), figS25, width = 8, height = 12, dpi = 300)
```

**Figure S25.** Phylogenetic tree (left panel) and species-level effect size estimates (right panel). No individual species showed a statistically significant deviation from the overall neutral effect of dispersal on fitness.
...
# Use females as the reference level for sex so the sex coefficient is the female–male comparison, rather than each sex relative to the combined "Both" category. 
dat$sex <- relevel(factor(dat$sex), ref = "Female")
...
small
ggsave(file.path(fig_dir, "s26.jpeg"), small, width = 6, height = 6, dpi = 300)
```
...
decline
ggsave(file.path(fig_dir, "s27.jpeg"), decline, width = 6, height = 6, dpi = 300)
```
...
bubble_plot(mod_comb,
            ...
            ylab = "Effect Size: Zr",
            g = TRUE)
ggsave(file.path(fig_dir, "s28.jpeg"), width = 6, height = 6, dpi = 300)
...
bubble_plot(mod_comb,
            ...
            ylab = "Effect Size: Zr",
            g = TRUE)
ggsave(file.path(fig_dir, "s29.jpeg"), width = 6, height = 6, dpi = 300)
...
leaveoneout
ggsave(file.path(fig_dir, "s30.jpeg"), leaveoneout, width = 10, height = 15, dpi = 300)
```
...
leaveoneout1
ggsave(file.path(fig_dir, "s31.jpeg"), leaveoneout1, width = 10, height = 15, dpi = 300)
```
...
# Save Fig S32 to file. funnel() is base-R graphics, so it needs an explicit
# device (last_plot()/ggsave do not apply); we redraw it inside a jpeg device.
jpeg(file.path(fig_dir, "s32.jpeg"), width = 6, height = 8, units = "in", res = 300)
funnel(mod1,
       yaxis="seinv",
       xlab = "Standarized residuals",
       ylab = "Precision (inverse of SE)",
