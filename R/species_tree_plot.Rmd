...
data_for_tree_plot<-left_join(data_for_tree, (df%>%mutate(tip.label = as.factor(gsub("_", " ", species_cleaned)))%>%select(tip.label, species_class)), by ="tip.label") 
...
p2 <- p + 
	geom_fruit(geom = geom_tile, mapping = aes(fill = dispersal_pattern), width = 1, offset = 0.085) + 
    	scale_fill_manual(values = c("#009E73", "#CC79A7", "black"), , name="Dispersal type")  # Create tiles to indicate which metric was used for this species
#p2

p3 <- p2 + new_scale_fill() + 
	geom_fruit(geom = geom_tile, mapping = aes(fill = fitness_measure), offset = 0.1, width = 1) + 
	scale_fill_manual(values = c("#E69F00", "#F0E442", "#0072B2"), name="Fitness measure")  # Create tiles to indicate which metric was used for this species
#p3

# Display number of effect sizes and species classes
p4 <- p3 + 
	new_scale_fill() + 
	geom_fruit(geom = geom_bar, mapping = aes(x = n_es, fill=species_class), stat = "identity", col = "gray97", orientation = "y", axis.params = list(axis = "x", text.angle = -90, hjust = 0, text.size = 3), grid.params = list(alpha = 0.35), offset = 0.085, pwidth = 0.55, alpha = 0.8) + 
	scale_fill_manual(values = c("#00BBDB","darkslategray4", "darkorchid", "chartreuse", "gray", "forestgreen"), name = "Class") + #"darkseagreen1", "deeppink", "darkred", "darkolivegreen2","#39568CFF", "darkgoldenrod2"
