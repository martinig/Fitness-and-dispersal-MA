...
#str(Species_info)
length(unique(Species_info$species)) #354
...
table((lengths(gregexpr("\\W+", Species_info$species)) + 1)) #20 records with >2 words species names (subspecies etc.)
...
Species_info <- Species_info %>%
  mutate(animal = 
  	case_when(species == "Spermophilus parryii plesius" ~ "Spermophilus parryii",
  		species %in% c("Strix occidentalis lucida", "Strix occidentalis caurina")  ~ "Strix occidentalis",
  		species == "Canis lupus arctos" ~ "Canis lupus",
  		species == "Lagopus lagopus scoticus" ~ "Lagopus lagopus",
  		species == "Aegolius funereus L." ~ "Aegolius funereus",
  		...
  		species == "Thryomanes bewickii, Junco hyemalis, Pipilo maculatus, Melospiza melodia" ~ "Passeriformes", #went with higher level
  		TRUE ~ species ))
...
length(unique(Species_info$animal)) #344
...
table(taxa$flags) # flags 18 names with problems (hidden, hybrid, incertae_sedis_inherited, infraspecific, infraspecific) - will need fixing
